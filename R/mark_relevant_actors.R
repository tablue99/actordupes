...
mark_relevant_actors <- function(dataframe, prepared_duplicate_data, min_similarity) {
  data <- dataframe |> 
    ...
    dplyr::mutate(relevant = dplyr::case_when(!is.na(first_mention) & relevant == FALSE & any_rel == TRUE ~ TRUE,
                                              is.na(first_mention) & is.na(duplicate) & relevant == TRUE & any_rel == FALSE ~ FALSE,
                                              relevant == TRUE & !is.na(duplicate) ~ FALSE,
                                              .default = relevant)) |>
    dplyr::select(-c(first_mention, last_name, stem_name, any_rel))
}